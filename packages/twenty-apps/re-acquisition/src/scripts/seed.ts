...
// CoreApiClient ships as a zero-arg stub and is code-generated against the live
// schema on publish, at which point its constructor takes ClientOptions. This
// alias keeps the file typechecking in both states without an `any` cast.
type CoreApiClientOptions = {
  url?: string;
  headers?: Record<string, string>;
};

const CoreApiClientWithOptions = CoreApiClient as unknown as new (
...
const main = async () => {
  // The client has no `apiKey` option — it reads a bearer token off the
  // Authorization header, falling back to TWENTY_APP_ACCESS_TOKEN/TWENTY_API_KEY.
  const client = new CoreApiClientWithOptions({
        ...
        __args: {
          data: {
            ...
            offerPrice: deal.offerPrice
              ? { amountMicros: toMicros(deal.offerPrice), currencyCode: 'USD' }
              : undefined,
            arv: deal.arv
